...
df["interest"] = df["interest"].apply(
    lambda x: [i.lower().strip() for i in str(x).replace(";", ",").split(",")]
)
df["skills"] = df["skills"].apply(
    lambda x: [
        s.lower().strip() for s in str(x).replace(";", ",").split(",") if s.strip()
    ]
)
...
print(
    f"Total with gender+grades: {2 + len(mlb_interest.classes_) + len(mlb_skills.classes_)}"
)
